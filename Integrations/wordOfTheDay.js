...
module.exports = async () => {
  try {
    ...
    if (data.results.length) {
      ...
      console.log(chalk.green(`word: ${data.word}`));
      console.log(chalk.green(`definition : ${def}`));
      console.log(chalk.green(`synonym : ${syn}`));
      console.log(chalk.green(`antonyms : ${ant}`));
      console.log(chalk.green(`examples : ${ex}`));
    } else {
      wordOfTheDay = chalk.red(`No Data Available for "${word}"`);
